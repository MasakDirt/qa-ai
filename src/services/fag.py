...
class FAGService(BaseService):
    ...
    async def get(self, question: str) -> str:
        logger.debug(f"Getting fag`s for question: {question}")
        embedded_question = await self._openai_client.get_embedding(question)
        ...
        return await self._openai_client.get_response(question, [fag.answer for fag in fags])
